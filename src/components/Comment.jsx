...
const Comment = () => {
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 댓글 삭제 axios
  const [{ loading3 }, deleteComment] = useAxios({ method: 'DELETE' }, { useCache: false, manual: true });

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 댓글 수정 axios
  const [{ loading4 }, putComment] = useAxios({ method: 'PUT' }, { useCache: false, manual: true });
  ...
  return (
    <div>
      ...
      <form onSubmit={formik.handleSubmit}>
        <input type="text" placeholder="댓글입력" name="comment" {...formik.getFieldProps('comment')} />
